...
static void media_thumbnail_parse (AppData& appdata, const std::string& ip, const json& data) {
...
static void media_url_parse (AppData& appdata, const std::string& ip, const json& data) {
    std::string urls;
...
static void media_last_parse (AppData& appdata, const std::string& ip, const json& data) {
    if(!data["value"]["data"].is_array()){
        std::cerr << "Invalid message from " << server << ": " << msg << std::endl;
...
static void query_set_all_parse (AppData& appdata, const std::string& ip, const json& data) {
    if(_camera_apply_all_feedback != NULL) _camera_apply_all_feedback();
}

static void query_get_parse (AppData& appdata, const std::string& ip, const json& data) {
    if(key == "query:getall"){
...
static void command_parse (AppData& appdata, const std::string& ip, const json& data) {
    if(!data["value"]["data"].is_array()){
        std::cerr << "Invalid message from " << server << ": " << msg << std::endl;
...
static bool vaildation (const std::string msg, json& target) {
    if(!json::accept(msg)) {
        std::cerr << "Invalid message from " << server << ": " << msg << "\n";
        std::cerr << "Parse error\n" <<;
        return false;
    }
    if(!data["key"].is_string()){
        std::cerr << "Invalid message from " << server << ": " << msg << "\n";
        std::cerr << "Key is not string\n";
        return false;
    }
...
void gopro_master_process_message(AppData& appdata, const std::string& ip, const std::string& msg) {
    json data;
    if(!vaildation(msg, data)) return;

    std::string key = data["key"].get<std::string>();
    if(key == "command:ip"){
        command_parse(appdata, ip, data);
    }
    else if(key == "query:get" || key == "query:getall"){
        query_get_parse(appdata, ip, data);
    }
    else if(key == "query:set"){
        
    }
    else if(key == "query:setall"){
        query_set_all_parse(appdata, ip, data);
    }
    else if(key == "media:lastmedia"){
        media_last_parse(appdata, ip, data);
    }
    else if(key == "media:url"){
        media_url_parse(appdata, ip, data);
    }
    else if(key == "media:info"){
        
    }
    else if(key == "media:list"){
        media_list_parse(appdata, ip, data);
    }
    else if(key == "media:thumbnail"){
        media_thumbnail_parse(appdata, ip, data);
    }
